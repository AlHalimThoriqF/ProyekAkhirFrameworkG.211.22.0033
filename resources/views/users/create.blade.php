...
<div class="card-body login-card-body">
    <p class="login-box-msg">{{ __('Add New User') }}</p>
    ...
    <form method="POST" action="{{ route('user.store') }}">
        ...
        <div class="row">
            <div class="col-12">
                <button type="submit" class="btn btn-primary btn-block">{{ __('Add User') }}</button>
            </div>
        </div>
    </form>
    <div class="mt-3">
        <p class="mb-0">
            <a href="{{ route('users.index') }}" class="text-center">{{ __('Back to User List') }}</a>
        </p>
    </div>
</div>
@endsection
